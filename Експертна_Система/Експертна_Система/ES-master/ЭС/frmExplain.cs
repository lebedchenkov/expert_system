        ...
        public frmExplain(ExpertSystem es)
        {
            ...
            foreach (Rule r in es.Rules.Vals)
            {
                if (r.Worked != RuleWork.No)
                {
                    ...
                    treeView2.Nodes[r.Name].Nodes.Add(r.Name + "ЯКЩО", "ЯКЩО");
                    treeView2.Nodes[r.Name].Nodes.Add(r.Name + "ТО", "ТО");
                    treeView2.Nodes[r.Name].Nodes.Add(r.Name + "ТК", "ТАК ЯК");
                    foreach (Fact f in r.Reasons)
                    {
                        treeView2.Nodes[r.Name].Nodes[r.Name + "ЯКЩО"].Nodes.Add(f.ToString());
                    }
            ...
            foreach (Rule r in es.WorkedRules)
            {
                if (r.Worked == RuleWork.Signifi)
                {
                    ...
                    treeView1.Nodes[r.Name].Nodes.Add(r.Name + "ЯКЩО", "ЯКЩО");
                    treeView1.Nodes[r.Name].Nodes.Add(r.Name + "ТО", "ТО");
                    treeView1.Nodes[r.Name].Nodes.Add(r.Name + "ТК", "ТАК ЯК");
                    foreach (Fact f in r.Reasons)
                    {
                        treeView1.Nodes[r.Name].Nodes[r.Name + "ЯКЩО"].Nodes.Add(f.ToString());
                    }
            ...
            listBox1.Items.Add("------ Встановлені факти ------");
